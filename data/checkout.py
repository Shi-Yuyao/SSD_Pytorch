...
class CheckoutDetection(data.Dataset):
    ...
    def __init__(self,
        ...
        self._annopath = os.path.join('%s', '%s', 'Annotations', '%s.xml')
        self._imgpath = os.path.join('%s', '%s', 'JPEGImages', '%s.jpg')
        self.ids = list()
        for (date, name) in image_sets:
            self._data = date
            for line in open(
                    os.path.join(self.root, date, name + '.txt')):
                self.ids.append((self.root, date, line.strip()))
    ...
    def _get_voc_results_file_template(self):
        ...
        filedir = os.path.join(self.root, 'results', 'CHECKOUT' + self._data,
                               'Main')
    ...
    def _write_voc_results_file(self, all_boxes):
        for cls_ind, cls in enumerate(CHECKOUT_CLASSES):
            ...
            with open(filename, 'wt') as f:
                for im_ind, index in enumerate(self.ids):
                    ...
                    for k in range(dets.shape[0]):
                        f.write(
                            '{:s}/{:s} {:.3f} {:.1f} {:.1f} {:.1f} {:.1f}\n'.format(
                                index[1], index[2], dets[k, -1], dets[k, 0] + 1,
    ...
    def _do_python_eval(self, output_dir='output'):
        ...
        if output_dir is not None and not os.path.isdir(output_dir):
            os.mkdir(output_dir)
        for i, cls in enumerate(CHECKOUT_CLASSES):
            if cls == '__background__':
                continue
